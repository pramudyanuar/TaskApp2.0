...
const Page = () => {
    ...
    const handleAddTask = async () => {
        ...
        const newTagsInput = prompt("Enter tags (comma-separated):");
        const newTags = newTagsInput ? newTagsInput.split(',') : [];
    ...
    const handleStatus = async (taskId: any, newStatus: string) => {
      try {
    ...
    const handleEditTitle = async (taskId: any, newTitle: string) => {
      try {
        const response = await fetch(`https://oprec-api.labse.in/api/task/${taskId}`, {
            method: 'PUT',
            headers: {
    ...
    const handleEditDesc = async (taskId: any, newdesc: string) => {
      try {
        const response = await fetch(`https://oprec-api.labse.in/api/task/${taskId}`, {
            method: 'PUT',
            headers: {
    ...
    const filteredTasks = tasks.filter(task => task.status !== 'deleted') as any[];
    const groupedTasks: { [key: string]: any[] } = filteredTasks.reduce((acc, task) => {
        acc[task.status] = [...(acc[task.status] || []), task] as any[];
        return acc;
    }, {});
